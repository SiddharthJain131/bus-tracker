...
# ================================================================================
# ⚠️  WARNING: This endpoint triggers system-wide photo restoration!
# 
# This will:
# - Restore ALL missing photos across students, teachers, parents, and admins
# - Regenerate corrupted or unreadable images
# - Generate AI-powered placeholder images using thispersondoesnotexist.com
# - Restore attendance scan photos from database records
# - Update ALL photo references in the database
# 
# This operation may take several minutes and will modify photos system-wide.
# Only elevated admins should trigger this process.
# ================================================================================

@api_router.post("/admin/restore_photos")
async def restore_all_photos_endpoint(current_user: dict = Depends(get_current_user)):
    """
    Admin-only endpoint to trigger system-wide photo restoration.
    
    ⚠️  WARNING: This is a system-wide operation that will:
    - Restore missing photos for all entities (students, teachers, parents, admins)
    - Restore missing attendance scan photos
    - Generate AI placeholders via thispersondoesnotexist.com for missing photos
    - Update database photo references
    
    Only elevated admins should use this endpoint.
    """
    # Check admin permissions
    if current_user['role'] != 'admin':
        raise HTTPException(status_code=403, detail="Only admins can restore photos")
    
    # Check if elevated admin
    if not current_user.get('is_elevated_admin', False):
        raise HTTPException(
            status_code=403, 
            detail="Only elevated admins can perform system-wide photo restoration"
        )
    
    try:
        # Import the restoration module
        import sys
        sys.path.insert(0, str(ROOT_DIR))
        from restore_all_photos import restore_all_photos as run_restore
        
        logging.info(f"Photo restoration triggered by {current_user['name']} ({current_user['email']})")
        
        # Run the restoration
        stats = await run_restore(generate_placeholders=True)
        ...
        response = {
            ...
            "summary": {
                ...
                "photos_verified": stats.total_verified,
                "placeholders_generated": stats.total_generated,
                "directories_created": stats.total_created,
                "database_updated": stats.total_updated
            },
            "by_entity": {
                entity_name: {
                    "processed": entity_stats['processed'],
                    "photos_verified": entity_stats['photos_verified'],
                    "placeholders_generated": entity_stats['photos_generated'],
                    "database_updated": entity_stats['database_updated']
                }
        ...
        if stats.errors:
            response['warnings'] = stats.errors[:10]  # Return first 10 errors
        
        logging.info(f"Photo restoration completed. Total processed: {stats.total_processed}")
        
        return response
    
    except Exception as e:
        logging.error(f"Photo restoration failed: {str(e)}")
