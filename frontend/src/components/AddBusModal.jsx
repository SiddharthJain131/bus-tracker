...
export default function AddBusModal({ open, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'Failed to create bus');
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-gradient-to-br from-orange-400 to-amber-600 rounded-full flex items-center justify-center">
              <BusIcon className="w-6 h-6 text-white" />
            </div>
            <DialogTitle className="text-2xl font-bold">Add New Bus</DialogTitle>
          </div>
        </DialogHeader>
